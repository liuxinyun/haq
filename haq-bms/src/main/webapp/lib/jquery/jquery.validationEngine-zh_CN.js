			...
			$.validationEngineLanguage.allRules = {
				...
				"fileType":{
					"func": function(field,rules,i,options){
						var typeArr = field.attr('accept').split(','); 
						var type = field.val().substring(field.val().lastIndexOf("."));
				...
				"double2":{
					"regex":/^(\d+)?(\.\d{1,2})?$/,
					"alertText":"* 无效的价格（支持两位小数） "
				},
				"gt":{
					"func":function(field,rules,i,options){
						var gtVal = $('#'+field.data('gt')).val();
						if(!gtVal) return true;
						return parseFloat(gtVal) <= parseFloat(field.val());
					},
					"alertText":"* 不能小于最小值 "
				},
				"lt":{
					"func":function(field,rules,i,options){
						var ltVal = $('#'+field.data('lt')).val();
						if(!ltVal) return true;
						return parseFloat(ltVal) >= parseFloat(field.val());
					},
					"alertText":"* 不能大于最大值 "
				}
				/**
